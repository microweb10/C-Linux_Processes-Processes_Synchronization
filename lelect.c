...
int main(int argc,char *argv[]){
	...
	semaforo=creaSEM(2,"clave");      // 0-Seccion Critica  1-MUTEX
	iniSEM(semaforo,0,1);
	iniSEM(semaforo,1,1);
	...
	for(i=0;i<procesos;i++){
		...
		case 0:
			...
			lectores=(int*)shmat(shmid,0,0);  // vinculamos la memoria compartida al proceso
			if(lectores==NULL) {*lectores=0;
					printf("lectores=%d\n",*lectores);}
			if(traza[i].etiqueta=='E'){
				...
				shmdt((char*)lectores);  // desvinculamos la memoria compartida del proceso
				exit(0);
			}
			else{
				sleep(traza[i].retardo);
				...
				P(semaforo,1);
				*lectores=*lectores+1; printf("lectores=%d\n",*lectores);
				if(*lectores==1) P(semaforo,0);
				V(semaforo,1);
